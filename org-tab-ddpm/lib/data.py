...
def num_process_nans(dataset: Dataset, policy: Optional[NumNanPolicy]) -> Dataset:
    ...
    # Ensure all arrays are float32 before isnan check
    dataset = replace(
        dataset,
        X_num={k: np.asarray(v, dtype=np.float32) for k, v in dataset.X_num.items()}
    )
    ...
    if policy == 'drop-rows':
        ...
        for k, v in X_num.items():
            nan_mask = nan_masks[k]
            if nan_mask.ndim == 2:
                # 2D array: (rows, features) — use col index to pick correct mean
                num_nan_indices = np.where(nan_mask)
                v[num_nan_indices] = np.take(new_values, num_nan_indices[1])
            else:
                # 1D array: flat — fill all NaNs with the single mean value
                v[nan_mask] = new_values if np.ndim(new_values) == 0 else new_values[0]
...
def normalize(
    ...
    return {k: normalizer.transform(_ensure_2d(v)) for k, v in X.items()}, normalizer


def cat_process_nans(X: ArrayDict, policy: Optional[CatNanPolicy]) -> ArrayDict:
    assert X is not None
    nan_masks = {k: np.asarray(v == CAT_MISSING_VALUE) for k, v in X.items()}
    if any(np.asarray(x).any() for x in nan_masks.values()):
        if policy is None:
            X_new = X
