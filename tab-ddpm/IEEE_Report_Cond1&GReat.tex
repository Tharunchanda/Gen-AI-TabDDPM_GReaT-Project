...
\title{Distribution-Regularized Diffusion Models for Constrained Synthetic Tabular Data Generation:\nA Comprehensive Comparative Study on Financial Customer Churn Datasets}

\author{
    \IEEEauthorblockN{Department of AI \& Machine Learning Research}
    \IEEEauthorblockA{GenAI-GREAT-MODEL \& Bank Churn Dataset Study \\ April 2026}
}

\maketitle

\begin{abstract}
Synthetic tabular data generation has become increasingly critical for addressing privacy preservation, data augmentation, and model evaluation challenges in financial machine learning applications. This paper presents a comprehensive two-phase empirical study evaluating the practical performance of synthetic customer data generation methodologies on a bank customer churn dataset from Botswana. In Phase 1, we conduct a detailed baseline evaluation of the Generative Regression Tree (GReaT) framework, employing an extensive suite of statistical fidelity metrics. Our analysis reveals a pronounced dichotomy in synthetic data quality: categorical features achieve excellent distributional alignment with real data (Jensen-Shannon Divergence mean: 0.2167), while numerical features exhibit substantial divergence (mean Wasserstein Distance: 93.8166) and generate unconstrained outliers that violate domain-specific value ranges.

To systematically address these numerical distribution pathologies, Phase 2 introduces and evaluates Condition-1 (Cond1)---a lightweight training-side regularization framework for Tabular Denoising Diffusion Probabilistic Models (TabDDPM). The proposed approach incorporates three key components: (1) soft distribution regularization during training to align predicted denoised numerics with target per-feature mean and standard deviation statistics, (2) optional soft constraint penalties for empirical quantile-bound enforcement, and (3) deterministic stratified evaluation for reproducible metrics. Experimental results demonstrate that Cond1 achieves well-calibrated synthetic data quality (mean Wasserstein Distance: 0.0363) while substantially reducing constraint violations to 9.24\%, thereby establishing a principled trade-off between marginal fidelity preservation and domain-required constraint satisfaction. The methodology is reproducible, hyperparameter-tunable, and generalizable to other tabular datasets.
\end{abstract}

\begin{IEEEkeywords}
synthetic tabular data, diffusion models, TabDDPM, GReaT, distribution regularization, evaluation metrics, constraint satisfaction, privacy-preserving data
\end{IEEEkeywords}
...
\begin{itemize}
    ...
    \item To rigorously quantify the distributional fidelity of GReaT-generated synthetic data, decomposed across numerical and categorical feature subsets, to identify specific pathologies.
...
\section{PHASE 1: BASELINE EVALUATION USING GReaT FRAMEWORK}
...
\begin{itemize}
    ...
    \item GReaT synthetic output: 4,995 records (5× augmentation factor)
...
\begin{table}[htbp]
\centering
\caption{Global Evaluation Metrics Summary (GReaT)}
...
\caption{Phase 1: Output matrix for Wasserstein distance across numerical features generated by GReaT.}
...
\caption{Phase 1: Output matrix for Jensen-Shannon divergence across categorical features generated by GReaT.}
\label{fig:great_jsd}
\end{figure}

\begin{table}[htbp]
\centering
\caption{Wasserstein Distance per Numerical Feature (GReaT)}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lll}
\toprule
...
\begin{table}[htbp]
\centering
\caption{Jensen-Shannon Divergence per Categorical Feature (GReaT)}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lll}
\toprule
...
\begin{itemize}
    \item \textbf{Categorical feature fidelity}: Excellent distributional alignment for demographic categorical features, particularly Gender (JSD = 0.0237), suggesting that GReaT's categorical modeling components are well-calibrated for balanced demographic distributions.
...
\begin{enumerate}
    ...
    \item \textbf{Limited model expressiveness:} GReaT's tree-based architecture may lack sufficient expressiveness to model complex multimodal and correlated structures present in financial customer populations.
    ...
    \item \textbf{Absent constraint enforcement:} The baseline GReaT architecture provides no mechanism for enforcing domain-specific value constraints, permitting unconstrained generation of implausible extremes.
\end{enumerate}
...
The images below document the baseline TabDDPM reference set used for comparison.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{baseline_evaluation/summary_dashboard.png}
\caption{Baseline evaluation dashboard summarizing overall fidelity diagnostics.}
\label{fig:summary_dashboard}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{baseline_evaluation/numerical_distributions.png}
\caption{Baseline numerical distributions across the churn features.}
\label{fig:numerical_distributions}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{baseline_evaluation/categorical_distributions.png}
\caption{Baseline categorical feature distributions for churn.}
\label{fig:categorical_distributions}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{baseline_evaluation/wasserstein.png}
\caption{Per-feature Wasserstein distance for the churn evaluation set.}
\label{fig:wasserstein}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{baseline_evaluation/jsd.png}
\caption{Per-feature Jensen-Shannon divergence for the churn evaluation set.}
\label{fig:jsd}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{baseline_evaluation/density_coverage.png}
\caption{Density and coverage summary for the churn evaluation set.}
\label{fig:density_coverage}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{baseline_evaluation/correlation_real.png}
\caption{Correlation heatmap for the real churn data.}
\label{fig:correlation_real}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{baseline_evaluation/correlation_synthetic.png}
\caption{Correlation heatmap for the synthetic churn data.}
\label{fig:correlation_synthetic}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{baseline_evaluation/correlation_diff.png}
\caption{Correlation difference heatmap between real and synthetic data.}
...
\begin{enumerate}
    ...
    \item \textbf{Downstream Utility Benchmarking}: Implement the comprehensive joint benchmark utility (scripts/joint\_benchmark.py): train RandomForest, XGBoost, and logistic regression classifiers on synthetic data; evaluate performance on real held-out test sets; measure AUC, F1-score, precision, recall. Compare Cond1 against baseline TabDDPM and GReaT.
