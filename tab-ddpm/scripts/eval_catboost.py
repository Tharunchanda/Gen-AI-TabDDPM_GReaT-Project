...
def train_catboost(
    ...
    if 'cat_features' not in catboost_config:
        ...
        # Fallback tuned configs can come from another dataset; keep only valid indices.
        valid_cat_features = [
            int(i) for i in catboost_config['cat_features'] if 0 <= int(i) < D.n_features
        ]
        if len(valid_cat_features) != len(catboost_config['cat_features']):
            print(
                'Warning: dropping out-of-range cat_features from CatBoost config. '
                f'Original={catboost_config["cat_features"]}, '
                f'valid={valid_cat_features}, n_features={D.n_features}'
            )
        catboost_config['cat_features'] = valid_cat_features
