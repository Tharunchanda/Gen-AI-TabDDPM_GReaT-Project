...
def sample(
    ...
    # Try to load EMA model if available (typically better quality)
    model_ema_path = model_path.replace('model.pt', 'model_ema.pt')
    if os.path.exists(model_ema_path):
        print(f"Loading EMA model from {model_ema_path}")
        model.load_state_dict(
            torch.load(model_ema_path, map_location="cpu")
        )
    else:
        print(f"No EMA model found at {model_ema_path}, using regular model")
