...
def get_catboost_config(real_data_path, is_cv=False):
    ...
    config_path = Path(f'tuned_models/catboost/{ds_name}_cv.json')
    if config_path.exists():
        return load_json(config_path)

    fallback_path = Path('tuned_models/catboost/default_cv.json')
    if fallback_path.exists():
        print(
            f'Warning: missing CatBoost config for dataset "{ds_name}"; '
            f'falling back to {fallback_path}.'
        )
        return load_json(fallback_path)

    raise FileNotFoundError(
        f'Could not find CatBoost config: {config_path} and fallback {fallback_path}.'
    )