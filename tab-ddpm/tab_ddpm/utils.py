...
def discretized_gaussian_log_likelihood(x, *, means, log_scales):
    ...
    log_cdf_plus = torch.log(cdf_plus.clamp(min=1e-12))  # Clamp to avoid log(0)
    log_one_minus_cdf_min = torch.log((1.0 - cdf_min).clamp(min=1e-12))  # Clamp to avoid log(0)
...
def sliced_logsumexp(x, slices):
    lse = torch.logcumsumexp(
        torch.nn.functional.pad(x, [1, 0, 0, 0], value=-1e9),  # Use -1e9 instead of -inf
        dim=-1
    )
