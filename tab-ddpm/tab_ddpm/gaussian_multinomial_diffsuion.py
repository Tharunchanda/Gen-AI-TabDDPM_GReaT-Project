...
class GaussianMultinomialDiffusion(torch.nn.Module):
    def __init__(
        ...
        self.distribution_weight = 0.0
        self.distribution_mean_weight = 1.0
        self.distribution_std_weight = 1.0

        if constraint_regularization is not None:
            weight = float(constraint_regularization.get('weight', 0.0))
            lower_bounds = constraint_regularization.get('lower_bounds', None)
            upper_bounds = constraint_regularization.get('upper_bounds', None)
            if weight > 0.0 and lower_bounds is not None and upper_bounds is not None:
                self.constraint_weight = weight
                self.register_buffer(
                    'constraint_lower_bounds',
                    torch.tensor(lower_bounds, dtype=torch.float32, device=device),
                )
                self.register_buffer(
                    'constraint_upper_bounds',
                    torch.tensor(upper_bounds, dtype=torch.float32, device=device),
                )

        if distribution_regularization is not None:
            dist_weight = float(distribution_regularization.get('weight', 0.0))
            target_mean = distribution_regularization.get('target_mean', None)
            target_std = distribution_regularization.get('target_std', None)
            if dist_weight > 0.0 and target_mean is not None and target_std is not None:
                self.distribution_weight = dist_weight
                self.distribution_mean_weight = float(distribution_regularization.get('mean_weight', 1.0))
                self.distribution_std_weight = float(distribution_regularization.get('std_weight', 1.0))
                self.register_buffer(
                    'distribution_target_mean',
                    torch.tensor(target_mean, dtype=torch.float32, device=device),
                )
                self.register_buffer(
                    'distribution_target_std',
                    torch.tensor(target_std, dtype=torch.float32, device=device),
                )
    ...
    def _constraint_loss(self, pred_xstart_num):
        if self.constraint_weight <= 0.0 or pred_xstart_num.shape[1] == 0:
            return pred_xstart_num.new_tensor(0.0)

        lower = self.constraint_lower_bounds.unsqueeze(0)
        upper = self.constraint_upper_bounds.unsqueeze(0)
        
        # Soft constraint using ReLU (linear penalty for violations)
        # This is more stable than quadratic and provides smooth gradients
        under_penalty = torch.relu(lower - pred_xstart_num)
        over_penalty = torch.relu(pred_xstart_num - upper)
        
        # Normalize by bound range to make loss scale-invariant
        bound_ranges = (upper - lower).clamp(min=1e-6)
        penalty_per_feature = (under_penalty + over_penalty) / bound_ranges
        
        return penalty_per_feature.mean()

    def _distribution_loss(self, pred_xstart_num):
        if self.distribution_weight <= 0.0 or pred_xstart_num.shape[1] == 0:
            return pred_xstart_num.new_tensor(0.0)

        batch_mean = pred_xstart_num.mean(dim=0)
        batch_std = pred_xstart_num.std(dim=0, unbiased=False).clamp(min=1e-6)

        target_mean = self.distribution_target_mean
        target_std = self.distribution_target_std.clamp(min=1e-6)

        mean_err = ((batch_mean - target_mean) / target_std) ** 2
        std_err = ((batch_std - target_std) / target_std) ** 2

        return self.distribution_mean_weight * mean_err.mean() + self.distribution_std_weight * std_err.mean()
    ...
    def mixed_loss(self, x, out_dict):
        ...
        if x_num.shape[1] > 0:
            ...
            pred_xstart_num = model_out_num
            if self.gaussian_parametrization == 'eps':
                pred_xstart_num = self._predict_xstart_from_eps(x_t=x_num_t, t=t, eps=model_out_num)
            constraint_loss = self._constraint_loss(pred_xstart_num)
            if self.constraint_weight > 0.0:
                loss_gauss = loss_gauss + self.constraint_weight * constraint_loss
            distribution_loss = self._distribution_loss(pred_xstart_num)
            if self.distribution_weight > 0.0:
                loss_gauss = loss_gauss + self.distribution_weight * distribution_loss
